...
vec4 SampleTriplanar(sampler2D tex, vec3 p, vec3 n, float s) {
    // Считаем веса смешивания на основе нормали
    vec3 blending = abs(n);
    blending = max((blending - 0.2) * 7.0, 0.0); // Делаем углы резкими, без мыла
    blending /= (blending.x + blending.y + blending.z); // Нормализуем

    // Умножение на s (triplanarScale) масштабирует текстуру в мире
...
vec3 TriplanarNormal(sampler2D tex, vec3 p, vec3 n, float s) {
    ...
    // ИНВЕРСИЯ ДЛЯ DIRECTX ТЕКСТУР: чтобы впадины были впадинами!
    tX.y = -tX.y; tY.y = -tY.y; tZ.y = -tZ.y;

    // Разворачиваем нормаль в зависимости от того, на какую сторону куба смотрим
    tX.z *= sign(n.x);
    tY.z *= sign(n.y);
    tZ.z *= sign(n.z);
